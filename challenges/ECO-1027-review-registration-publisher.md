...
- **Stage the diff before you start typing comments.** `git diff main..pr/registration-publisher-WIP -- apps/athlete-svc schemas` gives you the eleven files in this change. Read them once end-to-end before drafting feedback.
- **Cite concrete files when you flag something.** A review that says "this should follow the pattern in `<file>:<lines>`" is far more actionable than "this should follow the pattern."
