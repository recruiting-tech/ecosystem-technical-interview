...
This is the piece where compatibility decisions matter. The repo's
`make check-compat` target runs Schema Registry's compatibility check
against the on-disk schema; please use it.
...
> waits.
>
> Reminder: the apps in this monorepo are co-located *for the
> interview's convenience only*. In production they're four separate
> repos with four separate on-call rotations. Treat the `indexer/`
> directory as if you didn't have commit access.

## Discussion welcome

- `BACKWARD` vs `FULL` compatibility for this subject
- Optional with default vs required and bumped to `Camp.V2`
- Whether the producer needs a feature flag to wait for consumer rollout

## How to verify

- **`make check-compat`** is the primary safety net — it should print `✓ Camp.V1-value compatible` against the registered version. If you make an incompatible change (e.g. adding a required field with no default), it'll fail with Schema Registry's verbose explanation. That's the moment that proves your reasoning on optional vs. required.
...
- **End-to-end with a still-old indexer:** with `make api-svc` and `make indexer` running, POST a camp with `skill_level` and watch `/tmp/idx.log` (or wherever you redirected stderr). The indexer must consume cleanly even though its `campEventV1` struct doesn't know about the field — that's the proof that BACKWARD compatibility is real, not theoretical.
