...
# Preflight: wait briefly for api-svc, fail clearly if it never comes up.
ready=0
for _ in $(seq 1 15); do
  if curl -fsS -o /dev/null "${API}/camps?limit=1" 2>/dev/null; then
    ready=1
    break
  fi
  sleep 1
...
if [ "$ready" -ne 1 ]; then
  printf "\033[31m✗\033[0m api-svc not reachable at %s after 15s.\n" "$API" >&2
  printf "  Start it first — in another terminal: \033[36mmake api-svc\033[0m\n" >&2
  printf "  Or run the whole stack:               \033[36mmake dev\033[0m\n" >&2
  exit 1
fi
