...
Tests run against a real broker — but each test session gets its own
ephemeral Redpanda container (Kafka + Schema Registry) via testcontainers,
so dev compose state stays clean. See conftest.py for the bootstrap.
If you're tempted to swap in a Mock Producer here, please don't: every
test we've debugged in this codebase that "passed but broke in prod" was
a mocked broker.
"""
